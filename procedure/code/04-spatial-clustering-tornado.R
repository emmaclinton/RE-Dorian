...
# load GAtornado and GAbaseline data if not already loaded
GAtornado = readRDS(here("data","derived","private","GAtornado.RDS"))
GAbaseline = readRDS(here("data","derived","private","GAbaseline.RDS"))

GAtornado_sf = GAtornado %>%
...
tornado_by_county = GAtornado_sf %>%
  st_drop_geometry() %>%   # drop geometry / make simple table
  group_by(GEOID) %>%      # group by county using GEOID
  summarise(GAtornado = n())  # count # of tweets

counties = counties %>%
  left_join(tornado_by_county, by="GEOID") %>% # join count of tweets to counties
  mutate(GAtornado = replace_na(GAtornado,0))       # replace nulls with 0's

rm(tornado_by_county)

# Repeat the workflow above for baseline tweets

baseline_by_county = GAbaseline %>% 
...
counties = counties %>%
  ...
  mutate(nov = replace_na(base,0))

counties = counties %>%
  mutate(tornadorate = GAtornado / POP * 10000) %>%  # tornadorate is tweets per 10,000
  mutate(ndti = (GAtornado - base) / (GAtornado + base)) %>%  # normalized tweet diff
...
saveRDS(counties,here("data","derived","public","counties_tornado_tweet_counts.RDS"))
...
counties$locG = as.vector(localG(counties$tornadorate, listw = dwm, 
                                 zero.policy = TRUE))
...
ggplot() +
  ...
  labs(title = "Clusters of GA Tornado Twitter Activity")+
