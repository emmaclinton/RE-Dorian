...
packages = c("rtweet","tidycensus","tidytext","maps","RPostgres","igraph","tm", 
             "ggplot2","RColorBrewer","rccmisc","ggraph","here", "rgeos")
...
tornadoTweetsByHour <- ts_data(GAtornado, by="hours")
...
tornadoNetwork <- network_graph(GAtornado, c("quote"))
...
stop_words = stop_words %>% 
  ...
  add_row(word="shelter",lexicon = "Search") %>% 
...
tornadoWordPairs %>%
  filter(n >= 7 & !is.na(word1) & !is.na(word2)) %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = n, edge_width = n)) +
  geom_node_point(color = "darkslategray4", size = 3) +
  geom_node_text(aes(label = name), vjust = 1.8, size = 3) +
  labs(title = "Word Network of Tweets during Recent Tornado in GA",
       x = "", y = "") +
  theme_void()
...
counties = filter(counties,
                  STATEFP %in% c('01', '05', '29', '22', '47', '13', '12', '45', '37', '28' ))
...
install.packages(sf)
library(sf)
countyPts = st_join(GAtornado_sf, counties, joins=st_intersects)

countyPts <- lat_lng(GAtornado_sf,coords=c("coords_coords"))
...
ggplot() +
  geom_sf(data=counties, aes(fill=cut_number(DENSITY,4)), color="grey", stroke = 0.01)+
  scale_fill_brewer(palette="GnBu")+
  guides(fill=guide_legend(title="Population Density"))+
  geom_point(data = countiesPts, aes(x=lng,y=lat),
             colour = 'purple', alpha = .2) +
  labs(title = "Tweet Locations During Recent GA Tornado")+
